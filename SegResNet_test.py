...
def main(tempdir):
    ...
    saver = SaveImage(output_dir="./SegResNet", output_ext=".nii.gz", output_postfix="seg")
    ...
    model.load_state_dict(torch.load("SegResNet_best_metric_model_segmentation3d_array.pth"))
    model.eval()
    with torch.no_grad():
        ...
        np.save('SegResNet_dice_score.npy', dice_metric.aggregate().detach().cpu().numpy())
        #print(iou_metric.aggregate().detach().cpu().numpy())
        np.save('SegResNet_iou_metric.npy', iou_metric.aggregate().detach().cpu().numpy())
        #print(hd_metric.aggregate().detach().cpu().numpy())
        np.save('SegResNet_hd_metric.npy', hd_metric.aggregate().detach().cpu().numpy())
        dice_metric.reset()
        iou_metric.reset()
        hd_metric.reset()
